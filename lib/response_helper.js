...
resHelper.autoDetect = function(res, filepath) {
  var filetype = path.extname(filepath);
  if(filetype == '') {
    //if filetype is empty, default to plain
    filetype = 'plain';
  }
  if(filetype[0] === '.') {
    filetype = filetype.slice(1);
  }
  resHelper.extensionType(res, filetype);
};

resHelper.extensionType = function(res, type, next) {
  var contentTypeShortcut = {
    'plain': 'text/plain',
    'html': 'text/html',
    'xml': 'text/xml',
